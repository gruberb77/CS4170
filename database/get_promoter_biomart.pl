...
<Query  virtualSchemaName = "default" formatter = "FASTA" header = "0" uniqueRows = "0" count = "" datasetConfigVersion = "0.6" >
	...
	<Dataset name = "hsapiens_gene_ensembl" interface = "default" >
		<Filter name = "upstream_flank" value = "$length"/>
...
$ua->request($request, 
		 ...
		 if ($response->is_success) {
		 	...
		 	$data =~ s/\d//g; # removes numbers
			$data =~ s/^[^|]*|//; # removes gene symbol (from response)
			$data =~ tr /|//d; # removes extra pipes

			my @lines = split /\n/, $data; # removes the newline 
